...
def create_input_vertical(halo_len, image_size, comm_size, rank):
    image_height_local = int(image_size[0])
    image_width_local = int(image_size[1] / comm_size)  # use num_spatial_parts
...
def create_input_horizontal(halo_len, image_size, comm_size, rank):
    image_height_local = int(image_size[0] / comm_size)  # use num_spatial_parts
...
def create_input_square(halo_len, image_size, comm_size, rank):
    image_height_local = int(image_size[0] / math.sqrt(comm_size))
    image_width_local = int(image_size[1] / math.sqrt(comm_size))

    np_x = np.asarray(list(range(0, image_size[0] * image_size[1])), dtype=np.float32)
    np_x.resize(image_size[0], image_size[1])

    expected_output = np.pad(np_x, pad_width=halo_len, mode="constant")

    np_x.resize(1, 1, image_size[0], image_size[1])
    expected_output.resize(
        1, 1, image_size[0] + 2 * halo_len, image_size[1] + 2 * halo_len
    )
...
def create_input(halo_len, image_size, comm_size, rank, slice_method):
    if slice_method == "vertical":
        return create_input_vertical(halo_len, image_size, comm_size, rank)
    elif slice_method == "horizontal":
        return create_input_horizontal(halo_len, image_size, comm_size, rank)
    elif slice_method == "square":
        return create_input_square(halo_len, image_size, comm_size, rank)
...
def run_benchmark(rank, size, hostname):
    ...
    input_seq = np.asarray(
        list(range(0, image_size[0] * image_size[1])), dtype=np.float32
    )

    input_seq.resize(1, 1, image_size[0], image_size[1])
